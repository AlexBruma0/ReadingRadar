...
export const fetchCommentsByBook = createAsyncThunk('comments/fetchCommentsByBook', async (bookId, thunkAPI) => {
  const response = await fetch(`${import.meta.env.VITE_API_URL}/comments/book/${bookId}`);
  return response.json();
});

export const createCommentAPI = createAsyncThunk('comments/createComment', async ({ content, bookId }, thunkAPI) => {
...
export const updateCommentAPI = createAsyncThunk('comments/updateComment', async ({ commentId, content }, thunkAPI) => {
  const jwtToken = localStorage.getItem('jwtToken');
...
export const deleteCommentAPI = createAsyncThunk('comments/deleteComment', async (commentId, thunkAPI) => {
  const jwtToken = localStorage.getItem('jwtToken');
